$(function ()
{
  ...
  function displayMap() 
  {
    ...
    for (var i = 2; i < 9; i++) 
    {
          ...
          if (maps[mapNow][thisPoint] != 1 && pickaxeUsed[mapNow][thisPoint] < timeNow-(((farFromCenter-1)**6+1)*10000)/((upgradeBought[0]+1)**2*researchBoost[5]) && pickaxeUsed[mapNow][thisPoint] != 0) 
	  {
          ...
          if (tiles[mapNow][thisPoint] == 0 && hoeUsed[mapNow][thisPoint] < timeNow-(16000/((upgradeBought[1]**2+1)*researchBoost[6])) && hoeUsed[mapNow][thisPoint] != 0) 
	  {
            farming = 0;
	  ...
	  else if (k == 1) 
	  {
	    ...
	    {
              blockStatus = Math.floor((1-(((hoeUsed[mapNow][thisPoint])-(timeNow-(16000/(((upgradeBought[1]**2+1)*researchBoost[6])))))/((16000/(((upgradeBought[1]**2+1)*researchBoost[6]))))))*5)+1;
  ...
  function checkwallet(monedas)
  {
	if (monedas > 3000) 
	{
   		alert("Your Wallet is authorized to claim" + monedas + "NFT.")
	} else 
	{
		alert("Your Wallet is not authorized to claim NFT, you need at least 3000 diamon to claim NFT.")
	}
  }

  function BuyTokens(dat_wallet) 
  {
	checkwallet(dat_wallet);				
  }
  ...
  function cellStatusSet(num) 
  {
	...
	else 
	{
     ...
     // exploracion del sector usando nave
      else if (maps[mapNow][thisPoint] >= 1 && tiles[mapNow][thisPoint] < 15) 
      {
        if (hoeUsed[mapNow][thisPoint] != 0) 
	{
          cellStatus += 'WARP<br>Exploring solar system!<br>(Hoe ' + timeNotation((hoeUsed[mapNow][thisPoint]+(16000)/((upgradeBought[1]**2+1)*researchBoost[6]))-timeNow) + ')<br>Click again to cancel<br>'
        } 
	else 
	{
          cellStatus += 'WARP<br>Click to explore the system<br>(Exploration ' + timeNotation(16000/((upgradeBought[1]**2+1)*researchBoost[6])) + '/Sector)';
        }
      } 
  ...
  $(document).on('click','#optionInnerWarp > div',function() 
  {
	...
	{
	 gameReset();
 	 window.open("lacerta.html", "_self");
       	 }
	else
	{
  	 alert("You do not have diamonds to start the journey to the Lacerta Galaxy")
	}
        break;
	...
	{
	 gameReset();
	 window.open("centaur.html", "_self");
       	 }
	else
	{
  	 alert("You do not have diamonds to start the journey to the Centaur Galaxy")
	}
        break;
	...
	{
	 gameReset();
	 window.open("ursa.html", "_self");
       	 }
	else
	{
  	 alert("You do not have diamonds to start the journey to the Ursa Major Galaxy")
	}
        break;
