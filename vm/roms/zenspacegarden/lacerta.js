$(function ()
{
  ...
  function displayMap() 
  {
    ...
    for (var i = 2; i < 9; i++) 
    {
          ...
          if (maps[mapNow][thisPoint] != 1 && pickaxeUsed[mapNow][thisPoint] < timeNow-(((farFromCenter-1)**6+1)*10000)/((upgradeBought[0]+1)**2*researchBoost[5]) && pickaxeUsed[mapNow][thisPoint] != 0) 
	  {
          ...
          if (tiles[mapNow][thisPoint] == 0 && hoeUsed[mapNow][thisPoint] < timeNow-(60000/((upgradeBought[1]**2+1)*researchBoost[6])) && hoeUsed[mapNow][thisPoint] != 0) 
	  {
            farming = 0;
	  ...
	  else if (k == 1) 
	  {
	    ...
	    {
              blockStatus = Math.floor((1-(((hoeUsed[mapNow][thisPoint])-(timeNow-(60000/(((upgradeBought[1]**2+1)*researchBoost[6])))))/((60000/(((upgradeBought[1]**2+1)*researchBoost[6]))))))*5)+1;
  ...
  function cellStatusSet(num) 
  {
	...
	else 
	{
     ...
     // tiempos de exploracion warp
      {
        if (hoeUsed[mapNow][thisPoint] != 0) 
	{
          cellStatus += 'WARP<br>Exploring solar system!<br>(Hoe ' + timeNotation((hoeUsed[mapNow][thisPoint]+(60000)/((upgradeBought[1]**2+1)*researchBoost[6]))-timeNow) + ')<br>Click again to cancel<br>'
        } 
	else 
	{
          cellStatus += 'WARP<br>Click to explore the system<br>(Exploration ' + timeNotation(60000/((upgradeBought[1]**2+1)*researchBoost[6])) + '/Sector)';
        }
      } 
  ...
  $(document).on('click','#optionInnerWarp > div',function() 
  {
	...
	{
  	 alert("You do not have diamonds to start the journey to the Pegasus Galaxy")
	}
        break;
